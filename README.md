...
│   └── word-wolf/           ワードウルフ進行アプリ（4〜6人グループ対応）
...
| ワードウルフ進行アプリ | HTMLアプリ（PC・プロジェクタ） | 4〜6人グループで遊ぶワードウルフを進行。お題配り（番号順に長押しで確認）→お話しタイマー→指差し→ウルフ発表まで。人数混在に対応。 | [`apps/word-wolf/`](apps/word-wolf/) |
...
- ワードウルフ進行アプリ：`https://ueshima-blip.github.io/my-tools/apps/word-wolf/`
