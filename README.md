...
| アカウント確認ツール | HTMLアプリ（教職員用） | 生徒がアカウント／パスワードを忘れたとき、年・組・氏名（ひらがな）から Googleアカウント・パスワード・端末番号・暗証番号・まなびポケット学校コードを調べて知らせる。Excel名簿を読み込み、**パスワードで暗号化した「保護版HTML」を作成して配布**できる（生のExcelを共有フォルダに置かずに済む）。完全オフライン・外部送信なし。 | [`apps/account-lookup/`](apps/account-lookup/) |
