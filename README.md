...
│   └── account-lookup/      アカウント確認ツール（生徒アカウント・パスワード照会）
...
| アカウント確認ツール | HTMLアプリ（教職員用） | 生徒がアカウント／パスワードを忘れたとき、年・組・氏名（ひらがな）から Googleアカウント・パスワード・端末番号・暗証番号・まなびポケット学校コードを調べて知らせる。Excel名簿を読み込み、名簿を暗号化して埋め込んだ**配布用HTML（パスワードあり／なしを選択可）**を作成できる（生のExcelを共有フォルダに置かずに済む）。完全オフライン・外部送信なし。 | [`apps/account-lookup/`](apps/account-lookup/) |
...
- アカウント確認ツール：`https://ueshima-blip.github.io/my-tools/apps/account-lookup/`（※ パスワードを扱うため、名簿の取り扱いに注意。アプリ本体に生徒情報は含まれません）
