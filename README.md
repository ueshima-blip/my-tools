...
└── word/    Word で使うマクロ・ツール
    └── auto-ruby/           自動ルビ振り（ふりがな）配布パッケージ
```
...
| 自動ルビ振り（ふりがな） | Word（マクロ） | 漢字にふりがなを自動で振る Word マクロを、**コピペ不要で他の先生に配れる形**にまとめた配布パッケージ。インストール用バッチ・リボン定義・手順書・**印刷して渡せる使い方マニュアル(PDF)**つき。 | [`word/auto-ruby/`](word/auto-ruby/) |
...
`word/auto-ruby/` は、漢字にふりがなを自動で振るマクロを **他の先生へ配るためのパッケージ**です。配り方・受け取った先生の使い方は、同フォルダの `README.md` を参照してください（Windows 版 Word 専用）。
