...
def build_story():
    ...
    s.append(li("パソコン：配付された <b>席替えアプリ_オフライン版.html</b> を Google Chrome／Microsoft Edge などで開きます"))
    s.append(li("iPad：配付された <b>席替えアプリ_オフライン版.html</b> を Safari で開きます"))
    s.append(p(
    ...
    s.append(p("「この2人を離したい／近づけたい」といった条件を設定できます。2名を選んで距離を指定します。", "body"))
    s.append(li("<b>○席以上遠ざける</b>：2人の席の<b>間に他の席が○席以上</b>入ります（例：「2席以上」→ 間に2席以上）"))
    s.append(li("<b>○席以内に近づける</b>：2人の距離を○席以内にします（「1席以内」＝縦・横・斜めの隣）"))
    s.append(p(
        "距離の数え方：縦・横・<b>斜め</b>のいずれの方向も「1席」と数えます。"
        "斜めの隣どうし（机の角が接する席）は距離1なので、「1席以上遠ざける」だけでも斜め隣にはなりません。", "note"
    ))
    s.append(p(
        "条件が多すぎたり、互いに矛盾していたり、固定（ピン）と両立できないときは、"
        "席替えの<b>実行前に警告</b>が出ます。警告では「そのまま実行する（満たせない条件が出ます）」か"
        "「中止して条件を見直す」かを選べます。", "warn"
    ))
    ...
    s.append(make_table([
        ...
        ["iPadで使いたい", "Safari で 席替えアプリ_オフライン版.html を開いてください。動作確認済みです。"],
